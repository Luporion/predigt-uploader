...
### Produktive Settings und Vimeo-Bibliotheksbrowser (02.09.2026)

Die Textual-Einstellungen sind nun in Allgemein, Verarbeitung/Dateien, Vimeo, Prediger und Erweitert/Admin gegliedert und bleiben in kleinen Terminals scrollbar. Vimeo-Team- und Folder-IDs sind keine normalen Eingabefelder mehr. Ein gemeinsamer `VimeoFolderCatalog` bildet Hierarchie und Breadcrumbs ab; die Settings wählen den Standard-Zielordner per Namen aus. Der Browser markiert den aktuellen Ordner und kann erst nach einem eigenen Bestätigungsdialog einen Top-Level- oder Unterordner anlegen. Der neue Ordner wird nicht automatisch Zielordner, sondern muss ausdrücklich ausgewählt und gespeichert werden. Ein nicht mehr auffindbarer konfigurierter Ordner führt zu einer Warnung ohne Ersatzwahl.

Die Zielordnerwahl ist im Vimeo-Bereich jetzt als eigener breiter Button `Zielordner auswählen / ändern` unmittelbar unter dem lesbaren aktuellen Ordner erreichbar. Die ausdrückliche Bestätigung im Browser speichert Folder-ID und -Name sofort atomar; bloßes Navigieren, Abbrechen oder Erstellen eines Ordners ändert die Config nicht. Fehlgeschlagenes Speichern lässt die bisherige Auswahl erhalten. Späte Ergebnisse von Bibliotheks-, Settings- und Folder-Workern werden nach dem Verlassen des Screens verworfen, damit keine bereits ausgehängten Widgets mehr aktualisiert werden.

Die Vimeo-Bibliothek besitzt jetzt `Alle Videos` und `Ordner`. Alle Videos lädt `GET /users/{owner}/videos` weiterhin seitenweise und zeigt Ergebnisse sofort; lokale Suche und Sortierung nach Datum oder Titel arbeiten auf den bereits geladenen Daten. Die Ordneransicht lädt den Team-Folderkatalog einmal und pro geöffnetem Ordner nur dessen paginierte Videoliste. Vollständige Ergebnisse werden pro Owner/Ansicht/Folder nur für die aktuelle App-Sitzung gecached; `Neu laden` invalidiert den jeweiligen Cache. Nur ein von Vimeo ausdrücklich geliefertes HTTPS-Downloadobjekt aktiviert `Video herunterladen` und öffnet genau diesen Link im Browser.

Die Config-Persistenz behält beim Speichern unbekannte TOML-Abschnitte und Schlüssel. Die Predigerverwaltung kann Namen zusätzlich normalisiert und case-insensitiv sicher umbenennen; das freie, tastaturbedienbare Autocomplete in Schritt 5 bleibt erhalten.

### Früher Vimeo-Embed und Vimeo-Bibliothek (01.09.2026)

Nach der Vimeo-Videoanlage werden Remote-ID/-URI und ein vorhandener Vimeo-Link weiterhin sofort atomar gespeichert. Neu versucht `VimeoPublishingService` unmittelbar danach, `embed.html` über die bestehende Video-/oEmbed-/Player-Kette zu ermitteln. Ein früher Erfolg schaltet `Vimeo öffnen` und `Embed-Code kopieren` schon während des laufenden tus-Uploads frei. Ein vorübergehend fehlender Embed-Code oder API-Fehler ist dabei nicht fatal; nach der Übertragung und spätestens im bestehenden Abschlussabschnitt wird erneut versucht. Dateiübertragung, Vimeo-Verarbeitung und Embed-Verfügbarkeit bleiben in Textual getrennte Zustände.

Das Textual-Hauptmenü enthält nun eine echte, rein lesende `Vimeo-Bibliothek`. Sie validiert mit dem vorhandenen Service Token, Team-Owner und den konfigurierten Zielordner und lädt dessen Videos paginiert außerhalb des UI-Threads. Die Ansicht bietet lokale Titelsuche, Auswahl, Vimeo öffnen, Link/Embed kopieren und Detaildaten zu Datum, Dauer, Upload-/Transkodierungsstatus und Privacy. Downloadvarianten werden nur dann als Capability angezeigt, wenn Vimeo im API-Feld `download` explizite HTTPS-Links liefert; die aktuelle Version startet bewusst noch keinen Download und deutet `files`-/Player-URLs nicht um.

Die Bibliothek veröffentlicht jetzt bereits jede kumulativ geladene API-Seite an die TUI und zeigt – sofern Vimeo sie liefert – die Gesamtzahl. Bei rund 606 Videos muss deshalb nicht mehr auf die letzte Seite gewartet werden. Vollständige Ergebnisse werden pro Owner-/Folder-ID für den laufenden App-Prozess gecached; `Neu laden` invalidiert den Cache ausdrücklich. Die Suche weist während des Pagings darauf hin, dass sie zunächst nur bereits geladene Titel umfasst.

Gespeicherte Vimeo-IDs werden vor Wiederverwendung weiterhin remote geprüft. Neu wird ein explizites HTTP 404 nach erfolgreicher Konto-/Team-/Folder-Vorprüfung als extern gelöschtes Video behandelt: Nur der Vimeo-Teil des Aufnahme-States wird zurückgesetzt und ein frischer Upload wieder erlaubt. Timeouts, Auth-/Berechtigungsfehler, 429 und 5xx bleiben konservativ unverändert und blockieren eine Neuanlage, damit kein Duplikat entsteht.

Schritt 8 besitzt nun `Upload stoppen` mit sicher vorausgewählter Fortsetzen-Aktion im Bestätigungsdialog. Das Stoppsignal wird kooperativ zwischen Vimeo-Operationen und tus-Blöcken ausgewertet; laufende HTTP-Operationen werden nicht hart abgebrochen. Nach jedem PATCH wird zuerst der bestätigte Offset atomar gespeichert. Schema 5 führt dafür `step.status=stopped` ein und behält Video-ID, tus-Link und Resume-Daten. Ein erneuter Versuch prüft das Remote-Video und setzt den vorhandenen Upload fort; es erfolgt kein automatisches DELETE. Early-Link und Early-Embed werden zusätzlich als grüne Verfügbarkeitshinweise und kurz hervorgehobene Aktionen sichtbar.
...
Workflow-State-Schema 5 ergänzt den fortsetzbaren Status `stopped`; Schema 4 speicherte bereits `transcode_status` und `folder_status`, Schema 3 `upload_status`, `uploaded_at` und die eindeutigen `target_folder_*`-Felder. Ältere Zustände und Schema-2-Foldernamen bleiben lesbar. Schon eine vorhandene Video-URI verhindert einen zweiten Platzhalter; nach bestätigtem Remote-Upload bleiben Status, URI, ID, URL und Zeitpunkt auch dann erhalten, wenn erst die Folder-Zuordnung oder der Embed-Abruf scheitert.
...
Die vollständige automatische Suite umfasst jetzt 432 bestandene Tests. Der Windows-Systemcheck ist einschließlich Wizard, Textual, Vimeo-HTTP-Abhängigkeit, Windows-Credential-Zugriff und FFmpeg grün. CLI-Hilfe und der bestätigungslose Smoke-Test-Vorschauweg wurden ebenfalls ohne Netzwerk-Upload geprüft.
